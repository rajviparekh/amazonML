...
class ProductClassifier(object):
    ...
    def compile(self, tokenizer, glove_dir='./data/glove.6B/', embedding_dim=100, dropout_fraction=0.0, kernal_size=5, n_filters=128):
    ...
    def train(self, data, labels, validation_split=0.2, batch_size=256, epochs=2):
        ...
        # Train!
        self.save()
        checkpointer = ModelCheckpoint(filepath=r'C:\Users\Rajvi\Documents\CODING\Hackathon\Amazon ML 2021\dataset52a7b21\dataset\productner-master\productner-master\models\classifier.h5', verbose=1, save_best_only=False)
        self.model.fit(x_train, y_train, validation_data=(x_val, y_val), callbacks=[checkpointer],epochs=epochs, batch_size=batch_size)
    ...
    def evaluate(self, x_test, y_test, batch_size=256):
        ...
        for category in self.category_map:
            ...
            target_names[self.category_map[category]] = str(category)
